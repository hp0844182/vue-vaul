...
import type { ConfigTheme, ConfigThemes, DefaultThemeType, NextUIPluginConfig } from './types'
import { darkLayout, defaultLayout, lightLayout } from './default-layout'
import { baseStyles } from './utils/classes'

const DEFAULT_PREFIX = 'yilanui'
  ...
  for (const [themeName, { extend, layout, colors }] of Object.entries(themes)) {
    ...
    for (const [colorName, colorValue] of Object.entries(flatColors)) {
      ...
      try {
        ...
        resolved.colors[colorName] = ({ opacityVariable, opacityValue, ...others }) => {
...
/**
 * Generates a core plugin.
 *
 * @param {ConfigThemes} [themes] - The themes configuration.
 * @param {DefaultThemeType} defaultTheme - The default theme.
 * @param {string} prefix - The prefix for CSS variables.
 * @param {boolean} addCommonColors - Whether to add common colors to the colors config.
 * @return {unknown} The generated plugin.
 */
function corePlugin(
  themes: ConfigThemes = {},
  defaultTheme: DefaultThemeType,
      ...
      theme: {
        extend: {
          ...
          fontSize: {
            ...
            title: [`var(--${prefix}-font-size-title)`, `var(--${prefix}-line-height-title)`],
            mainBody: [`var(--${prefix}-font-size-mainBody)`, `var(--${prefix}-line-height-mainBody)`],
            desc: [`var(--${prefix}-font-size-desc)`, `var(--${prefix}-line-height-desc)`],
          },
          borderRadius: {
...
export function nextui(config: NextUIPluginConfig = {}): ReturnType<typeof plugin> {
  const {
    themes: themeObject = {},
    defaultTheme = 'light',
    layout: userLayout,
